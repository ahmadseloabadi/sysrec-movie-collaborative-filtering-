...
def get_popular_movies(data, jumlah):
    ...
    popular_movies_info['similarity'] = random.random()
...
def get_top_n_recommendations_by_title(title, n_jumlah):
    ...
    try:
        ...
        movie_recs = [
            (
                model.trainset.to_raw_iid(inner_id),# ini untuk movie id
                model.sim[movie_inner_id, inner_id],# ini untuk simlarity
                model.predict(model.trainset.to_raw_uid(movie_inner_id), model.trainset.to_raw_iid(inner_id)).est #ini untuk pred
            )
